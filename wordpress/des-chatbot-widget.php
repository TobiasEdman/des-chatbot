...
add_action('admin_init', function () {
    register_setting('des_chatbot_options', 'des_chatbot_api_url', [
        'type' => 'string',
        'default' => 'https://des-chatbot.icedc.se/api/chat',
                ...
                <tr>
                    ...
                    <td>
                        <input type="url" id="des_chatbot_api_url" name="des_chatbot_api_url"
                               value="<?php echo esc_attr(get_option('des_chatbot_api_url', 'https://des-chatbot.icedc.se/api/chat')); ?>"
                               class="regular-text" />
...
function des_chatbot_render_widget() {
    ...
    if ($rendered) return;
    $rendered = true;
    $api_url = esc_url(get_option('des_chatbot_api_url', 'https://des-chatbot.icedc.se/api/chat'));
    ?>
<link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&display=swap" rel="stylesheet">
<style>
:root{--des-dark:#1a4338;--des-light:#cff8e4;--des-peach:#fdd5c2;--des-red:#ff826c;--des-black:#171717;--des-radius:12px}
#des-chat-toggle{position:fixed;bottom:24px;right:24px;z-index:99999;width:60px;height:60px;border-radius:50%;background:var(--des-dark);color:var(--des-light);border:none;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,.25);display:flex;align-items:center;justify-content:center;transition:transform .2s,background .2s}
#des-chat-toggle:hover{transform:scale(1.08);background:#15352c}
#des-chat-toggle:focus-visible{outline:3px solid var(--des-light);outline-offset:3px}
...
#des-chat-panel{position:fixed;bottom:96px;right:24px;z-index:99999;width:400px;height:500px;max-height:80vh;max-width:calc(100vw - 32px);background:#fff;border-radius:var(--des-radius);box-shadow:0 8px 32px rgba(0,0,0,.18);display:none;flex-direction:column;overflow:hidden;font-family:'Space Grotesk',Arial,system-ui,sans-serif}
#des-chat-panel.visible{display:flex}
.des-chat-header{background:var(--des-dark);color:var(--des-light);padding:14px 18px;font-size:15px;font-weight:600;display:flex;align-items:center;gap:10px;flex-shrink:0}
.des-chat-header svg{width:22px;height:22px;fill:currentColor;flex-shrink:0}
.des-chat-messages{flex:1;overflow-y:auto;padding:16px;display:flex;flex-direction:column;gap:10px;background:#eafcf0}
.des-msg{max-width:82%;padding:10px 14px;border-radius:var(--des-radius);font-size:14px;line-height:1.55;word-wrap:break-word}
.des-msg a{color:var(--des-dark);text-decoration:underline}
.des-msg a:hover{color:var(--des-red)}
.des-msg code{background:rgba(26,67,56,.08);padding:1px 5px;border-radius:4px;font-size:13px}
.des-msg pre{background:var(--des-dark);color:var(--des-light);padding:10px 12px;border-radius:8px;overflow-x:auto;margin:6px 0;font-size:13px}
...
.des-msg-user{background:var(--des-dark);color:var(--des-light);align-self:flex-end;border-bottom-right-radius:4px}
.des-msg-bot{background:#fff;color:var(--des-black);align-self:flex-start;border-bottom-left-radius:4px;box-shadow:0 1px 3px rgba(26,67,56,.1)}
.des-typing{align-self:flex-start;background:#fff;padding:10px 18px;border-radius:var(--des-radius);border-bottom-left-radius:4px;box-shadow:0 1px 3px rgba(26,67,56,.1);display:none;gap:5px;align-items:center}
.des-typing.visible{display:flex}
.des-typing span{width:7px;height:7px;background:var(--des-light);border-radius:50%;animation:desBounce 1.2s infinite}
...
.des-chat-input textarea:focus{border-color:var(--des-dark)}
.des-chat-input button{background:var(--des-dark);color:var(--des-light);border:none;border-radius:8px;width:40px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:background .15s}
.des-chat-input button:hover{background:#15352c}
.des-chat-input button:disabled{opacity:.5;cursor:not-allowed}
.des-chat-input button:focus-visible,.des-chat-input textarea:focus-visible{outline:2px solid var(--des-light);outline-offset:1px}
...
.des-chat-footer a{color:var(--des-dark);text-decoration:none}
